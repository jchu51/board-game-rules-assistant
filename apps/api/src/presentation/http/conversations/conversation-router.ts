...
export class ConversationRouter {
  ...
  constructor(private readonly service: ConversationService, actorService: ActorService) {
    this.router = Router();
    this.router.use(createActorMiddleware(actorService));
  ...
  private list = async (_req: Request, res: Response, next: NextFunction) => {
    try {
      return res.json(await this.service.list(res.locals.actor));
    } catch (error) {
      next(error);
    }
  };
  ...
  private remove = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      await this.service.delete(res.locals.actor, id.data);
