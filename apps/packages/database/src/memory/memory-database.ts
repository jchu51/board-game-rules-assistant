  ...
  return {
    identity: {
      ...
      async deleteExpiredGuestSessions({ now: cutoff }) {
        const expiredIds = [...guestSessions.values()]
        ...
        const conversationIds = new Set(
          [...conversations.values()]
            .filter(({ guestSessionId }) =>
              guestSessionId !== null && expiredIds.includes(guestSessionId))
            .map(({ id }) => id),
        );
        ...
        for (let index = messages.length - 1; index >= 0; index--) {
          if (conversationIds.has(messages[index]!.conversationId)) messages.splice(index, 1);
        }
        for (let index = citations.length - 1; index >= 0; index--) {
          if (messageIds.has(citations[index]!.messageId)) citations.splice(index, 1);
        }
    ...
    conversations: {
      async createConversation({ id, actor, gameId, title }) {
        ...
        if (!games.has(gameId)) {
          throw new PersistenceNotFoundError("game");
        }
        const guestSession = actor.kind === "guest"
          ? guestSessions.get(actor.guestSessionId)
          : undefined;
        if (actor.kind === "guest" && !guestSession) {
      ...
      async listOwnedConversations({ actor }) {
        return [...conversations.values()]
          .filter((conversation) => ownsConversation(conversation, actor))
          .sort((a, b) =>
